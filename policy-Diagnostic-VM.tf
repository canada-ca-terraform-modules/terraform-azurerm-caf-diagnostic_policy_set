...
/*
resource "azurerm_policy_assignment" "Deploy-Diagnostics-VM" {
  # count                = var.deployOptionalFeatures.deny_publicips_on_nics ? 1 : 0
  name                 = "Deploy-Diagnostics-VM"
  location             = var.log_analytics_workspace.location
  scope                = data.azurerm_subscription.primary.id
  policy_definition_id = azurerm_policy_definition.Deploy-Diagnostics-VM.id
  display_name         = "Deploy-Diagnostics-VM"
...
resource "azurerm_policy_remediation" "Deploy-Diagnostics-VM" {
  name                 = lower("Deploy-Diagnostics-VM-policy-remediation")
  scope                = azurerm_policy_assignment.Deploy-Diagnostics-VM.scope
  policy_assignment_id = azurerm_policy_assignment.Deploy-Diagnostics-VM.id
