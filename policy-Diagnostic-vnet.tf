...
/*
resource "azurerm_policy_assignment" "Deploy-Diagnostics-VNET" {
  # count                = var.deployOptionalFeatures.deny_publicips_on_nics ? 1 : 0
  name                 = "Deploy-Diagnostics-VNET"
  location             = var.log_analytics_workspace.location
  scope                = data.azurerm_subscription.primary.id
  policy_definition_id = azurerm_policy_definition.Deploy-Diagnostics-VNET.id
  display_name         = "Deploy-Diagnostics-VNET"
...
resource "azurerm_policy_remediation" "Deploy-Diagnostics-VNET" {
  name                 = lower("Deploy-Diagnostics-VNET-policy-remediation")
  scope                = azurerm_policy_assignment.Deploy-Diagnostics-VNET.scope
  policy_assignment_id = azurerm_policy_assignment.Deploy-Diagnostics-VNET.id
